...
export class G1World {
  ...
  private previous = new Float32Array(29)
  ...
  async init(signal?: AbortSignal): Promise<void> { this.session = await loadPolicy(this.asset, signal) }
  ...
  async step(command: Command): Promise<void> {
    ...
    const obs = buildObservation(cfg, this.data.qpos, this.data.qvel, this.previous, command, this.episodeStep)
    ...
    // No base/joint pinning. Exactly four torque-controlled steps per inference.
    for (let sub = 0; sub < 4; sub++) {
      this.data.ctrl.set(pdTorques(cfg, target, this.data.qpos, this.data.qvel))
      this.data.xfrc_applied.fill(0)
      if (this.forceSteps > 0) {
        this.data.xfrc_applied.set(this.force, this.manifest.torso_body * 6)
        this.forceSteps--
      }
    ...
    if (this.data.qpos[2] < .35) this.fallen = true
  }
  ...
  frame(): RobotFrame {
    ...
    return { bodyPos: Float64Array.from(this.data.xpos), bodyQuat: Float64Array.from(this.data.xquat),
      ...
      height: this.data.xpos[3 * this.manifest.torso_body + 2], fallen: this.fallen, force: [...this.force] }
  }
